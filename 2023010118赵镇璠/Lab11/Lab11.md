...
| 项目                   | 你的填写 |
| ---------------------- | -------- |
| `client` underlay 地址 |172.26.197.35 |
| `vpn` underlay 地址    |	192.0.2.1|
| `client` wg0 地址      |10.10.1.0/24|
| `vpn` wg0 地址         |10.10.2.1/24|
| `server` 地址          |192.0.2.1:51820|
...
| 失败的直接原因                   |AllowedIPs 改为10.30.0.0/24后，WireGuard 自动删除去往10.20.0.0/24的隧道路由，访问目标网段无路由，流量无法进入 wg 隧道转发 |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
>答：握手报文走底层 underlay 物理网络，不受 AllowedIPs 改动影响，所以 wg show 仍显示隧道握手正常；业务流量依靠 AllowedIPs 生成的 overlay 隧道路由转发，AllowedIPs 修改后 10.20.0.0/24 对应路由被删除，业务报文无隧道路由无法入隧道转发导致访问失败；握手和业务数据分属 underlay、overlay 两套不同传输路径。
...
| 协议                     |UDP|ICMP |
| 是否能看到 ICMP/HTTP 内容 |否|是|
| 为什么内外两层看到的不同 |underlay 是外层封装：原始 ICMP 报文被 WireGuard 加密，再封装 UDP 头部 + 底层 IP 头传输| wg0 是解封装后的内层接口，剥离外层 UDP/IP 与加密，还原出原始明文 ICMP 报文 |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
>答：（1）outer (underlay) 对应外层信封（信封皮，只有收发地址，看不到信纸内容）；inner (wg0) 对应内部信纸（原始明文业务数据）。（2）安全意义：原始业务报文全程加密封装在 UDP 载荷中在公网传输，中间人抓取外层流量只能看到底层 IP 与 UDP 头，无法解密获取内层明文数据，实现传输数据加密防护。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：（1）两件事：①自动生成内核路由，把目标网段流量导向 wg 隧道接口；②报文准入过滤（访问控制），仅目的 IP 在 AllowedIPs 网段的报文才允许通过隧道收发。（2）优先影响路由：wg-quick 依据 AllowedIPs 自动添加 / 删除系统路由，路由决定流量是否进入隧道，路由失效就无法传输，因此路由优先级高于访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手与业务分属两套传输路径：握手报文走底层 underlay 物理 IP（192.0.2.x），不受 AllowedIPs 与隧道路由影响，只要底层网络互通即可完成保活握手；业务流量依赖 AllowedIPs 生成的 overlay 隧道路由，路由缺失就无法进隧道转发，因此隧道在线但 curl 失败，二者相互独立。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：AllowedIPs=0.0.0.0/0 代表所有 IP 全量路由进隧道，风险：①本机全部流量强制从 VPN 隧道转发，消耗对端带宽；②对端被攻陷后，本机全量流量被劫持窃取；③易造成路由环路、全网断网；④无法精细化管控访问目标，失去 VPN 访问隔离能力。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 是非对称密钥鉴权，隧道建立需要双向密钥校验：客户端用自身私钥 + 对端公钥加密，服务端用自身私钥 + 客户端公钥解密；仅拿到服务端私钥，缺少客户端私钥无法生成合法签名，无法冒充 client 协商密钥、建立隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：（1）外层源地址：192.0.2.2（underlay 底层接口 IP）；（2）抓包看不到内层内容：WireGuard 对原始内层报文全程加密，外层 UDP 载荷是密文，抓包只能获取外层 IP/UDP 头部，无解密密钥无法还原明文。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①地址隐藏防护：外层只暴露底层物理 IP，隐藏内部业务网段私网地址，规避内网网段泄露；②数据加密防护：内层原始报文加密封装，链路抓包无法窃取业务明文，实现传输数据保密。
