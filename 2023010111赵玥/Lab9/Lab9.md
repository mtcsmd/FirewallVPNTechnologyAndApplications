...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` | 成功|返回目录页面HTML |
| `guest -> dmz:8080` |失败 |超时 |
| `guest -> office` | 失败|无法访问 |
...
> 答：不会。因为 REJECT 会终止匹配流程，数据包不会再走到后面的 LOG 规则。
...
```text
May 21 08:55:47 goodpartner kernel: GUEST-TO-DMZ: IN=veth-fw-guest OUT=veth-fw-dmz MAC=2a:24:23:c8:ad:00:f2:f3:c3:e2:71:50:08:00 SRC=10.0.2.2 DST=10.0.3.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=25192 DF PROTO=TCP SPT=39462 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0
```
...
> 答：合理，因为我们只给拦截规则加了 LOG。要记录放行流量，需要在 ACCEPT 规则前也加上 LOG 规则。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可以看到源 IP、目的 IP、协议、端口、进入 / 流出接口，能完整还原 “谁在访问谁的什么服务”。
...
> 答：可以快速分类、筛选、统计不同类型的安全事件，方便审计与排查。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：日志暴涨、占满磁盘、影响系统性能，甚至导致系统不稳定。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 | 5|
| `--limit-burst 5` 的含义 | 允许一次性最多记录五条日志|
...
> 答：独立生效。LOG 负责记录，REJECT 负责拦截，互不影响。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：有本质区别。不记录但仍然拦截，说明攻击被阻止，安全防护有效；只是日志被限流，不代表安全失效。
...
| :------- | :----- | :------- | :---------- |
| 1 | ACCEPT|ctstate ESTABLISHED,RELATED | 无|
...
| 过滤命令 | journalctl -k --grep "GUEST-TO-DMZ" --no-pager|
