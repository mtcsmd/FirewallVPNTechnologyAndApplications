...
| 项目                         | 你的填写 |
| ---------------------------- | -------- |
| `AllowedIPs` 的值            |    client：10.20.0.0/24；vpn：10.10.10.1/32      |
| 是否能访问 `10.20.0.2:8080`  |   是       |
| `ip route` 中 wg0 相关的条目 |   10.20.0.0/24 dev wg0 scope link；10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1       |
...
| 去 `10.20.0.0/24` 的路由是否消失 |   消失       |
| 是否还能访问 `10.20.0.2:8080`    |  不能        |
| 失败的直接原因                   |   client 没有去往 10.20.0.0/24 的路由，报文不送入 wg0 隧道，直接走本机默认路由丢弃，无法进入 VPN 加密通道       |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：握手基于底层192.0.2.x的 UDP 链路，不受AllowedIPs影响，隧道状态正常；业务流量依赖AllowedIPs生成的路由，路由失效报文不进隧道，访问失败。二者是两套独立路径。
截图：
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |   192.0.2.2（client 底层公网 IP）                                 |          |192.0.2.2（client 底层公网 IP）
| 看到的目的地址           |   192.0.2.1（vpn 底层公网 IP）                                 |          |10.20.0.2（业务服务器 IP）
| 协议                     |  UDP                                  |          |ICMP/HTTP(TCP)
| 是否能看到 ICMP/HTTP 内容 |    不可见，全加密十六进制                                |          |可见完整明文
| 为什么内外两层看到的不同 |  VPN 封装：内层原始 IP 报文整体加密，封装为外层 UDP 报文在公网传输，vpn 解密后还原原始内层报文                                  |          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：outer（underlay 底层抓包）= 外层信封：只看得见信封的收发地址（192.0.2.x底层公网 IP、UDP51820 端口），信封内部的信纸（真实数据）被加密封死，无法查看。
inner（wg0 虚拟网卡抓包）= 内部信纸：拆开封套后的原始内容，能看到真实内网 IP（10.10.x.x）、ICMP/HTTP 明文数据。
安全意义
① 数据机密性：内网通信载荷全程加密，链路窃听者拿到外层数据包也读不到业务明文；
② 地址隐藏防护：真实内网网段、两端内网 IP 被封装隐藏，公网链路无法探测内网拓扑，规避嗅探、定向攻击。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：①出口路由（发包）：wg-quick 自动把网段写入内核路由表，目的 IP 匹配则报文送入 wg0 隧道加密转发；②入口白名单（收包）：对端解密后的内层报文源 IP 不在 AllowedIPs 列表直接丢弃。因为路由是报文能否进入隧道的第一道门槛，因此优先影响路由，再做入站访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：WireGuard 握手报文使用底层 underlay 的 UDP（Endpoint 配置的公网 IP: 端口）传输，不受隧道内层路由和 AllowedIPs 管控；业务流量依赖系统路由 + AllowedIPs 路由规则决定是否进隧道，二者承载在不同寻址体系，因此握手在线但业务不通。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：本机所以全网流量全部强制进入 VPN 隧道：1. 客户端全部上网流量经过 VPN 网关，网关带宽耗尽；2. 一旦 VPN 网关故障，客户端断网；3. 网关侧被攻陷后可劫持客户端全部网络流量。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：WG 是双向公私钥校验：vpn 用 client 公钥校验报文合法性；client 用 vpn 公钥校验网关身份。伪造客户端必须持有client 私钥签名报文，仅有 vpn 私钥只能解密收到的数据，无法伪造客户端签名，不能建立隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：报文被 WireGuard 加密封装成 UDP 数据包，原始内层 IP 与载荷全部加密，公网链路只能抓取封装后的外层 UDP 包，无法解密所以看不到内层内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①数据机密性：原始应用载荷全加密，链路窃听无法窃取明文；②地址隐匿性：内网 IP 被封装隐藏，中间人无法获知内网网段、通信两端真实内网地址，规避嗅探与拓扑探测。
