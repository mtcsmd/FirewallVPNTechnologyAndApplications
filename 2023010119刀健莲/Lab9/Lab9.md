...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |	成功 |执行 curl 后返回了完整的 HTML 页面，显示目录列表，请求被正常转发并得到响应 |
| `guest -> dmz:8080` |失败 |执行 curl 后提示 Couldn't connect to server，连接被防火墙拒绝，无法访问目标端口 |
| `guest -> office` |失败 |执行 curl 后提示 Couldn't connect to server，请求被防火墙拒绝，无法访问目标网段 |
...
> 答：不会被命中。
原因是 iptables 的规则是按从上到下的顺序匹配的：当数据包匹配到前面的 REJECT 规则时，会直接被丢弃，不会继续往下匹配后面的 LOG 规则，因此 LOG 规则永远不会被触发。
简单来说，数据包被 REJECT 拦截后，流程就终止了，不会再走到 LOG 规则这一步。
...
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |失败（curl 提示无法连接） |是 |GUEST-TO-DMZ: |
| `guest -> office` | 失败（curl 提示无法连接） |是|GUEST-TO-OFFICE:|
| `office -> dmz:8080` |成功 | 否（无日志）|无 |

**从终端 D 复制一条实际日志行：**

```text
May 21 09:24:20 DESKTOP-B6DBPGB kernel: GUEST-TO-DMZ: IN=veth-fw-guest OUT=veth-fw-dmz MAC=2e:10:ec:bf:cc:9e:da:a7:4d:57:97:4c:08:00 SRC=10.0.2.2 DST=10.0.3.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=38196 DF PROTO=TCP SPT=49564 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0 
```
...
> 答：合理。
因为我们只给 被拒绝（REJECT）的流量配置了 LOG 规则，放行（ACCEPT）的流量没有配置任何日志规则，所以不会产生日志，这完全符合防火墙规则的设计逻辑。如果想记录所有被放行的流量，在对应的 ACCEPT 规则之前，添加一条匹配条件完全相同的 LOG 规则，即可记录所有被放行的流量。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：从日志里完全可以还原出 “谁在访问谁的什么服务”，而且能读出丰富的网络细节。
...
| 你使用的过滤命令 |grep "GUEST-TO-DMZ"、sudo journalctl -k --grep "GUEST-TO-DMZ" --no-pager、wc -l` 统计日志条数 |

**简答：**

1. `--log-prefix` + `grep` 组合，相当于给不同事件打了不同"标签"。在真实场景中这有什么实际价值？

> 答：在真实场景中，--log-prefix + grep 的组合就像给不同类型的网络事件打上了专属 “标签”，能让管理员在海量的内核日志中，快速、精准地过滤出特定访问行为的日志，大幅提升日志排查与审计效率，既方便对不同业务或访问策略的命中情况进行独立统计和分析，也能在发生安全事件时，快速定位违规访问、异常流量的来源与行为，为故障排查和安全响应提供关键依据。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：如果防火墙每秒拦截上千个包，不加限制地写 LOG，会导致内核日志被大量重复记录的数据包信息填满，造成日志文件快速膨胀、占用大量磁盘空间，甚至引发磁盘写满的风险；同时高频的日志写入会消耗大量 CPU 和 I/O 资源，影响防火墙的转发性能，严重时可能导致系统卡顿或服务响应变慢，也会让管理员难以在海量日志中定位关键信息，增加故障排查和安全审计的难度。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 | 5|
| `--limit-burst 5` 的含义 | 日志规则的初始 “令牌桶” 容量，允许一次性记录最多 5 条突发日志，之后按 --limit 速率限制写入|
| 未记录的访问是否仍然被 REJECT | 是，日志限制不影响 REJECT 规则，所有访问依然会被拦截|

**简答：**

1. `--limit` 只控制了 LOG 的写入频率，但后面的 REJECT 仍对每个包生效。这说明 LOG 和 REJECT 在逻辑上是什么关系？

> 答：LOG 和 REJECT 在逻辑上是相互独立、互不影响的关系，它们是两个独立的规则目标：
LOG 只是一个 “副作用”，它的限流只控制日志是否写入，不会影响数据包的匹配和转发流程；而 REJECT 是一个终止性动作，只要数据包匹配到规则，就会被直接拦截，无论前面的 LOG 是否触发、是否被限流，都不影响 REJECT 的执行。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：两者有本质区别：“没记录就当没发生” 意味着访问行为既未被拦截也未被日志记录，系统对违规流量完全无感知、无防护、无追溯，安全事件证据链断裂；而 “仍然拦截但不记录” 是指访问行为已被防火墙成功阻断，仅因日志限流等策略未写入日志，核心防护目标已达成。在安全审计中，这个区别至关重要，因为审计的核心是还原真实安全事件全貌，若误将 “拦截但不记录” 等同于 “没发生”，会导致管理员错误评估系统面临的攻击态势，无法发现持续的威胁并调整防护策略，也无法满足合规审计对访问控制有效性的验证要求，只有结合规则计数器和日志记录综合判断，才能准确评估安全防护状态。
...
| 测试项 | 结果 |
| :----- | :--- |
| `office -> dmz:8080` |成功（正常返回 HTML 目录页面） |
| `guest -> dmz:8080` |失败（curl 提示无法连接到服务器 |
| `guest -> office` |失败（curl 提示无法连接到服务器 |
...
| :------- | :----- | :------- | :---------- |
| 1 |ACCEPT |所有源 / 目的，状态 RELATED,ESTABLISHED |无 |
| 2 |ACCEPT |源：10.0.1.0/24，目的：10.0.3.2，TCP 目的端口：8080 |无 |
| 3 |LOG |源：10.0.2.0/24，目的：10.0.3.2，TCP 目的端口：8080 |GUEST-TO-DMZ: |
| 4 |	REJECT |源：10.0.2.0/24，目的：10.0.3.2，TCP 目的端口：8080 |无 |
| 5 |LOG |源：10.0.2.0/24，目的：10.0.1.0/24 |GUEST-TO-OFFICE: |
| 6 |	REJECT |源：10.0.2.0/24，目的：10.0.1.0/24 |无 |

| 测试 | 访问结果 | 是否出现 LOG | 日志前缀 |
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |失败（被 REJECT 拦截） |是 | GUEST-TO-DMZ:|
| `guest -> office` | 失败（被 REJECT 拦截）|是 |GUEST-TO-OFFICE: |
| `office -> dmz:8080` |成功（被 ACCEPT 放行）	| 否| 无|
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：因为 dmz 是对外提供服务的公开区域，而 office 是内部办公网络，二者安全等级不同，不能混在同一网段。从本实验的规则效果来看：我们为 guest -> dmz:8080 配置了 “先记录日志再拒绝” 的规则，而 guest -> office 也被单独拒绝。如果 dmz 和 office 在同一网段，防火墙就无法通过目的地址区分流量，只能用更复杂的端口或协议规则来区分，一旦规则出现疏漏，外部攻击者就可能通过访问 dmz 上的公开服务，直接渗透到同一网段的 office 内部主机，导致内部办公数据泄露。而将二者划分在不同网段后，我们可以通过源 / 目的网段，为 dmz 配置公开访问的放行规则，同时为 office 配置严格的访问控制策略，实现 “公开服务可被访问、内部网络被隔离保护” 的安全目标，就像本实验中 dmz 能被 office 正常访问，而 office 始终被外部 guest 隔离一样。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：guest 网络一般应与办公网隔离，核心原因是访客网络的安全不可控，与办公网混连会直接威胁内部数据安全：访客设备通常不受企业 IT 管控，可能存在恶意软件、漏洞或被植入攻击工具，一旦接入办公网，就可以直接访问内部文件服务器、业务系统和敏感数据；同时，混连也会让攻击者通过访客网络作为跳板，绕过防火墙直接渗透内部办公网。结合本实验来看，我们为 guest -> office 配置了单独的 REJECT 规则，让访客完全无法访问办公网，这正是隔离的体现 —— 如果不隔离，访客流量会直接到达办公网，无法通过网段区分并拦截，访客的任何异常访问都可能对内部办公系统造成安全威胁。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：必须保留，因为它允许已建立连接的响应包通过防火墙。
如果去掉，客户端发起请求后，服务器的回包会被拦截，所有访问都会失败，无法正常通信。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：LOG 与 ACCEPT/DROP/REJECT 最本质的区别是：LOG 只是一个 “无副作用的日志记录动作”，它只会记录数据包信息，不会改变数据包的转发流程；而 ACCEPT/DROP/REJECT 是终止性动作 ，会直接决定数据包的最终命运（放行、丢弃或拒绝）。LOG 必须放在 REJECT 前面，是因为规则是按顺序匹配的：数据包先命中 LOG 规则被记录日志，再继续匹配后续的 REJECT 规则被拦截。如果把 LOG 放在 REJECT 后面，数据包在匹配到 REJECT 时就已经被丢弃，永远不会走到后面的 LOG 规则，也就无法记录下被拦截的访问行为了。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：这是合理的设计，而非疏漏。原因在于，日志本身是一种消耗系统资源的操作，通常只会对需要重点监控的异常、违规或风险行为进行记录，而对合法、可信的访问行为默认不记录，以此减少日志冗余和系统开销。在本实验中，office -> dmz 是被明确允许的合法业务访问，无需额外记录；而 guest -> dmz 和 guest -> office 是被拦截的违规访问，属于需要重点审计的风险行为，因此才配置了 LOG 规则。这种 “只记录异常、不记录合法流量” 的策略，既满足了安全审计的核心需求，也避免了日志被大量正常访问淹没，是防火墙日志配置中非常典型的做法。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：两者有本质区别。--limit 控制日志频率后，超出限制的包仍被 REJECT 但不写入日志，只是日志记录被限流了，但访问控制的安全策略始终在生效，攻击行为已被成功阻断，只是部分记录未被写入日志；而 “只要不记录就当没发生” 意味着既无日志记录，也无访问控制，违规流量直接通过，安全事件无迹可寻也无防护。前者是 “拦截了但少记录”，后者是 “没拦截也没记录”，这在安全审计和防护效果上完全不同，不能混淆。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：从网络分区和日志审计两个角度，VPN 用户应先接入 DMZ 区域，而非直接接入办公网：网络分区角度：DMZ 是专为对外服务和外部接入设计的隔离区域，本身就是为了放置需要被外部访问的服务（如 VPN 网关），可以避免外部流量直接进入办公网。VPN 用户先接入 DMZ 中的网关，再通过严格的访问控制规则，按需放行到办公网的特定资源，实现 “外部接入 - 隔离中转 - 受控访问” 的三层防护，防止 VPN 用户直接暴露办公网内部网段。日志审计角度：将 VPN 网关部署在 DMZ 后，所有 VPN 接入行为（如登录、访问请求）都会在 DMZ 网关处集中产生日志，便于统一审计和溯源；同时，从 DMZ 到办公网的访问流量，也能通过防火墙规则配置单独的日志记录，清晰区分 VPN 用户与内部用户的访问行为，避免外部接入流量与内部办公流量混杂，大幅提升安全事件排查的效率和准确性。
