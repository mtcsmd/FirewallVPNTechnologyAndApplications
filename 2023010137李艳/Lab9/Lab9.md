...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功 | 用 curl 访问能正常返回 DMZ 的网页内容，DMZ 的服务日志显示 HTTP 200，规则中该条 ACCEPT 规则的包计数已增加|
| `guest -> dmz:8080` | 失败| 用 curl 访问返回 “Couldn't connect to server”，连接被直接拒绝，DMZ 服务日志无访问记录|
| `guest -> office` | 失败|用 curl 访问无法建立连接，连接被防火墙直接拒绝，无任何响应 |
...
> 答：不会被命中。
原因：iptables 规则是从上到下按顺序匹配的，数据包一旦匹配到 REJECT 规则，就会被直接拒绝并终止处理流程，不会继续往下匹配后面的 LOG 规则。所以必须把 LOG 放在 REJECT 之前，才能记录下被拒绝的数据包日志。
...
> 答：这是合理的。因为我们的 LOG 规则只配置在了被拒绝的流量（guest 相关）上，没有给 office -> dmz 的放行规则添加 LOG 动作，所以放行的连接不会触发日志。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：日志中可以读出完整的访问信息，包括：
源 IP（SRC）和目标 IP（DST）：能知道是哪个主机发起的访问、访问的目标是谁
协议（PROTO）和目标端口（DPT）：能知道访问的是什么服务（如 TCP 8080）
源端口（SPT）、TTL、SYN 标志等网络层 / 传输层细节
以示例日志为例，我们可以还原出：10.0.2.2（guest 主机）正在访问 10.0.3.2（dmz 主机）的 TCP 8080 端口服务，请求类型是 TCP 连接的第一次握手（SYN包）。
...
> 答：这种 “标签化日志” 的组合在真实场景中有很高的实用价值：
快速分类定位问题：通过不同前缀可以快速区分不同类型的安全事件（如外部攻击、内部异常访问），不用在海量日志里逐条排查。
简化告警与分析：可以针对特定前缀的日志设置告警（如GUEST-TO-DMZ日志激增），快速响应潜在的安全威胁。
便于审计与合规：清晰的标签能帮助管理员追溯特定规则的命中情况，满足安全审计和合规检查的要求。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：不加限制地记录所有被拦截的包会导致严重的问题：
日志文件极速膨胀：大量重复日志会快速占满磁盘空间，甚至导致系统崩溃。
系统性能下降：频繁的磁盘写入操作会占用大量 CPU 和 IO 资源，影响防火墙的正常转发和处理性能。
关键信息被淹没：大量无效日志会掩盖真正的攻击或异常事件，反而不利于问题排查。
因此，生产环境中必须通过 --limit 等参数对日志记录进行限速，避免日志刷屏。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5 |
| `--limit-burst 5` 的含义 |日志记录的初始 “令牌桶” 容量，代表最多允许一次性连续记录 5 条日志，超出的部分会被丢弃 |
| 未记录的访问是否仍然被 REJECT | 是，所有 10 次访问都被拒绝，只是日志只记录了 5 次|

**简答：**

1. `--limit` 只控制了 LOG 的写入频率，但后面的 REJECT 仍对每个包生效。这说明 LOG 和 REJECT 在逻辑上是什么关系？

> 答：LOG 和 REJECT 是相互独立的两个动作，逻辑上互不影响。
LOG 只是一个 “记录动作”，它不会改变数据包的流向，即使日志被限制不写，数据包依然会继续往下匹配后续规则。
REJECT 是 “终止动作”，会直接拒绝数据包，但不会影响前面 LOG 规则的匹配与否。
它们是独立执行的两个步骤，--limit 只会限制 LOG 规则的匹配频率，不会影响 REJECT 规则的正常生效。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：两者有本质区别。
“没记录就当没发生” 意味着数据包没有被防火墙处理，既没有被记录也没有被拦截，属于规则漏配，会直接导致安全隐患。
“仍然拦截但不记录” 是数据包被防火墙正常拦截，只是日志记录被限速丢弃，安全策略依然生效，只是审计信息不完整。
在安全审计中，这个区别非常重要：
前者是安全漏洞，攻击者的访问没有被任何控制，可能导致入侵成功。
后者是日志优化策略，安全防护本身没有问题，只是为了避免日志刷屏做了限速处理，不影响实际防护效果。
审计时需要区分这两种情况，避免把 “限速导致的日志缺失” 误判为 “安全策略失效”，也不能把真正的漏配当成正常的日志限制处理。
...
| `dmz` 地址 |10.0.3.2/24 |
| `fw` 三个接口地址 |10.0.1.1/24（连 office）、10.0.2.1/24（连 guest）、10.0.3.1/24（连 dmz） |
...
| 2 |ACCEPT |	-s 10.0.1.0/24 -d 10.0.3.2 -p tcp --dport 8080 |无 |
| 3 |LOG |	-s 10.0.2.0/24 -d 10.0.3.2 -p tcp --dport 8080 |GUEST-TO-DMZ: |
| 4 |REJECT |	-s 10.0.2.0/24 -d 10.0.3.2 -p tcp --dport 8080 | 无|
...
| 项目 | 你的填写 |
| :--- | :------- |
| `GUEST-TO-DMZ` 总条数 |7（之前 2 条 + 循环新增 5 条） |
| `GUEST-TO-OFFICE` 总条数 |1 |
| 过滤命令 |sudo journalctl -k --grep "GUEST-TO-" --no-pager |
| 加 `--limit` 后触发 10 次，日志条数 |5（受 --limit-burst 5 限制，只记录了前 5 条） |
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：如果 dmz 和 office 在同一网段，攻击者一旦攻破 dmz 里的公开服务，就能直接访问同一网段的办公内网，相当于绕过了防火墙的访问控制。在本实验中，我们通过规则拒绝了 guest 网段直接访问 office 网段的流量，同时只允许 office 访问 dmz 的服务，实现了双向隔离；如果两者在同一网段，这种精细化控制就会失效，办公内网会完全暴露给 dmz 的安全风险。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：guest 网段通常是给访客、外来设备或低信任度用户使用的，安全风险远高于办公网。如果不隔离，一旦 guest 网段中存在恶意设备或被攻击者攻陷，就能直接访问办公网中的敏感数据、服务器，导致企业核心数据泄露。在本实验中，我们通过 REJECT 规则直接阻断了 guest 到 office 的所有访问，就是为了防止低信任区域影响高信任区域。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：这条规则允许已建立连接的返回流量通过，是网络双向通信的基础。如果去掉，office 主机主动访问 dmz 服务的响应数据包会被防火墙当成 “未知连接” 直接丢弃，导致正常的访问失败。在本实验中，去掉这条规则后，即使我们放行了 office 到 dmz 的请求，也无法收到服务的响应，访问会直接超时。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：LOG 只是一个 “记录动作”，不会改变数据包的流向，也不会中断后续规则的匹配；而 ACCEPT/DROP/REJECT 会直接改变数据包的状态，其中 DROP/REJECT 会中断后续规则匹配。因此 LOG 必须放在 REJECT 前面：如果 REJECT 在前，数据包会被直接丢弃，后续的 LOG 规则永远不会被匹配，也就无法记录被拒绝的访问日志。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：这是合理的，不是疏漏。我们的实验场景中，只给被拦截的 guest 访问配置了 LOG 规则，目的是记录异常访问行为；而 office 到 dmz 的正常访问没有配置 LOG，是为了避免大量正常日志刷屏，同时也符合 “只记录异常行为” 的安全审计原则。如果需要记录放行流量，只需要在对应的 ACCEPT 规则前添加 LOG 规则即可。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。“只要不记录就当没发生” 意味着数据包既没有被记录也没有被拦截，属于安全策略失效，会直接导致入侵风险；而 “仍然拦截但不记录” 是数据包被正常拒绝，只是日志记录被限速丢弃，安全策略依然生效，只是审计信息不完整。在本实验中，即使日志被限制，10 次访问依然全部被拒绝，说明防护效果不受影响，只是日志数量被控制在了 burst 限制内。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：VPN 用户应该先接入 DMZ 区域，而不是直接接入办公网。
网络分区角度：VPN 用户属于外部接入的低信任用户，和 guest 网段类似，直接接入办公网会带来安全风险；先接入 DMZ 区域，可以通过防火墙对 VPN 用户到办公网的访问进行精细化控制，只开放必要的服务。
日志审计角度：在 DMZ 区域对 VPN 用户的访问配置 LOG 规则，可以完整记录所有外部接入的访问行为，便于后续安全审计和异常排查；如果直接接入办公网，日志会分散在办公网设备中，难以统一管理。
