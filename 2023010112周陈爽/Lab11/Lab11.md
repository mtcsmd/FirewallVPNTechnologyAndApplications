...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |     是，显示 1 minute,7 seconds ago（距离上次握手 1 分 7 秒）     |
| 是否看到 `transfer` 计数               | 是，两端均有收发字节统计：客户端：收 92B、发 392B；服务端：收 244B、发 92B         |
| client 端的 `allowed ips` 显示的是什么 |     10.20.0.0/24（代表访问目标 10.20.0.0/24 网段的流量全部走 WireGuard 隧道转发）     |
...
| 是否能访问 `10.20.0.2:8080`  |    是      |
| `ip route` 中 wg0 相关的条目 |  10.20.0.0/24 dev wg0 proto kernel scope link src 10.10.10.1        |
...
| `latest handshake` 是否还在      |  存在        |
| 去 `10.20.0.0/24` 的路由是否消失 |   是       |
| 是否还能访问 `10.20.0.2:8080`    |   不能访问       |
| 失败的直接原因                   | 客户端无去往10.20.0.0/24的隧道路由，访问目标网段的数据包无法送入 WireGuard 隧道封装转发         |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
隧道在线但业务失败原因
WireGuard 握手报文与业务流量受不同规则管控：
握手过程：依靠底层物理 underlay 地址（192.0.2.x）传输，不受AllowedIPs配置约束，只要两端公钥、底层 IP 端口正常即可完成密钥协商，因此wg show能查询到握手记录、隧道状态正常在线。
业务访问：业务 IP 报文转发由AllowedIPs自动生成的内核路由控制；修改AllowedIPs=10.30.0.0/24后，系统自动删除去往10.20.0.0/24的隧道路由，访问10.20.0.2的数据包无法匹配路由、不能送入 wg0 隧道封装转发，最终访问失败。
路径结论
握手、业务数据是两套不同传输路径
握手数据包：走底层物理链路（underlay 网段）
业务 IP 数据包：走VPN 隧道 overlay 链路，由 AllowedIPs 生成的路由决定是否进隧道
...
| 协议                     |                              UDP      |ICMP          |
| 是否能看到 ICMP/HTTP 内容 |                          看不到，内层报文被加密封装在 UDP 载荷里          |    能直接看到完整 ICMP 载荷内容      |
| 为什么内外两层看到的不同 |     WireGuard 做IP 加密封装：原始内网 ICMP 报文在 wg0 出接口被加密，外层新增 UDP + 底层 IP 头部，从 underlay 网卡发出，因此两层报文头部、协议、载荷完全不一样                               | wg0 是封装前原始明文报文，还没做 UDP 加密封装，保留原生 ICMP 头部与内网 IP         |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
outer (underlay 抓包)=外层信封：只能看见信封收发地址（底层 192.0.2.x）、信封载体 (UDP)，信纸（原始 ICMP 数据）被加密封在信封内部无法查看；
inner (wg0 抓包)=信纸原文：能看到原始内网 IP、ICMP 报文内容，是封装前的明文业务数据。
安全意义：公网传输只暴露底层 UDP 和外层 IP，业务报文全程密文，窃听者无法解析内网地址与业务数据，实现通信加密保护。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：第一件：内核路由控制：自动生成 / 删除去往对应网段的隧道路由，决定目标 IP 流量是否送入 wg0 隧道；
② 第二件：报文入站访问过滤：peer 收到报文时，校验源 IP 是否在 AllowedIPs 网段，不在则丢弃报文，充当访问控制。
原因：路由优先级在先，数据包先发路由查表决定走不走隧道，只有能进入隧道的报文才会触发入站 ACL 校验，因此优先作用是路由，表象是访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手报文基于底层 underlay 物理 IP（192.0.2.x）UDP 传输，不受 AllowedIPs 与内核路由约束，只要公私钥匹配、底层网络可达就能完成密钥协商，隧道状态在线；
业务 IP 流量由 AllowedIPs 生成的内核路由管控，修改 AllowedIPs 后目标网段无隧道路由，业务报文无法进隧道转发。
二者传输路径、管控规则分离，因此握手正常但业务访问失败。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：路由风险：本机所有 IP 流量全部默认走 WireGuard 隧道，极易造成全网流量被引流，本地断网；
安全风险：对端 peer 可以接收本机任意源 IP 报文，失去网段访问隔离，一旦对端被攻陷，本机全网段暴露，横向渗透风险极高。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。
WireGuard 是双向非对称验签：节点用对端公钥验签、自身私钥签名。攻击者仅有服务端私钥，没有客户端私钥，无法生成合法客户端签名报文；服务端使用攻击者伪造报文的签名和客户端公钥校验会失败，无法完成握手。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：underlay 底层物理 IP（实验里 192.0.2.2）；
看不到内层原因：WireGuard 对原始内网 IP+ICMP 报文全程加密，原始数据作为 UDP 载荷被密文封装，抓包只能获取外层 UDP + 底层 IP 头部，载荷密文无法逆向解析明文。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：隐私隐藏：公网链路隐藏真实内网网段（10 段），仅暴露底层物理 IP，规避内网拓扑泄露；
数据加密：内层业务报文全密文传输，链路抓包无法窃取 ICMP/HTTP 明文数据，防窃听篡改。
