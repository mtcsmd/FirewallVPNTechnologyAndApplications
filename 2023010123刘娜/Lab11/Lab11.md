...
| `latest handshake` 是否还在      |是          |
| 去 `10.20.0.0/24` 的路由是否消失 |是          |
| 是否还能访问 `10.20.0.2:8080`    |  否        |
| 失败的直接原因                   |          |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：1.wg show隧道显示存活，但业务访问失败的原因：WireGuard 的隧道握手、保活报文基于 ** 底层underlay互通的UDP地址（192.0.2.1:51820）** 传输，该链路不受AllowedIPs参数控制，因此peer握手状态持续有效，wg show能够正常显示隧道在线。
但业务访问的IP报文依赖内核路由表：修改AllowedIPs=10.30.0.0/24后，wg-quick删除了10.20.0.0/24指向wg0的路由，客户端访问10.20.0.2的数据包没有匹配隧道路由，不会被送入WireGuard封装转发，最终业务访问失败。
2.隧道握手和业务数据传输分属两套独立路径，二者使用两套完全不同的路径：握手/保活流量路径：使用underlay底层网络，以两端公网IP+UDP端口（192.0.2.2↔192.0.2.1:51820）直接传输，和AllowedIPs、上层隧道路由无关；业务数据流量路径：依赖内核 IP 路由 + AllowedIPs 规则，只有目标网段匹配 AllowedIPs、对应路由指向 wg0，数据包才会被封装进 WireGuard 隧道传输。
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             | 192.0.2.2（client 底层物理地址）                                   | 10.10.10.1（客户端隧道内网地址）         |
| 看到的目的地址           | 192.0.2.1（vpn 底层物理地址，目的端口 51820）                                   |10.20.0.2（server 业务内网地址）          |
| 协议                     |UDP（WireGuard 封装外层传输协议）                                    |ICMP（原始内层业务协议）          |
| 是否能看到 ICMP/HTTP 内容 |不能，载荷为加密密文，仅十六进制乱码，无法解析原始报文内容                                    |能，明文展示 ICMP echo request/reply、HTTP 请求等原始载荷          |
| 为什么内外两层看到的不同 | underlay 是 WireGuard外层封装报文：原始 IP+ICMP 报文被整体加密，封装在 UDP 报文里跨底层网络传输；                                   | wg0 是解密后的内层原始报文：vpn 设备解密 UDP 外层头部，剥离加密，还原出原本的内网 IP 与业务报文。         |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：外层 underlay 抓到的 UDP 封装报文对应信封，信封上只写两端底层的收发地址（192.0.2.x），信封内部的信纸被加密密封，无法直接查看；
内层 wg0 接口抓到的解密后原始 ICMP/HTTP 报文对应信纸，拆开信封解密后才能看到信纸上真实的内网源目地址与业务数据内容。=
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：第一件：路由生成wg-quick根据配置里的AllowedIPs自动在本机内核添加路由：目标网段匹配AllowedIPs的流量，路由指向wg0 网卡，决定哪些流量会进入隧道封装，这是优先生效的路由层面动作。第二件：报文准入校验（访问控制）对端收到隧道解密后的内层 IP 包时，校验报文源 IP 是否落在本端 Peer 的 AllowedIPs 网段内，不在范围内的数据包直接丢弃，实现访问过滤。原因：系统发包优先遵循内核路由表，若目标网段不在 AllowedIPs，不会生成隧道路由，报文根本不会送入 wg0、无法抵达对端，因此路由先生效；只有流量成功进入隧道抵达对端后，才会触发 AllowedIPs 的源地址校验，所以看似访问控制，实际先影响路由。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：第一，隧道握手 / 保活报文走 underlay 底层 UDP 链路：握手基于两端底层 underlay 地址（192.0.2.1/2.2+UDP51820 端口）传输，报文不受 AllowedIPs、内核业务路由控制，只要底层网络互通就能完成密钥握手，因此wg show会持续显示 latest handshake，隧道状态在线。第二，业务 curl 流量依赖 AllowedIPs 生成的内核路由：业务访问10.20.0.2需要匹配对应隧道路由，修改 AllowedIPs 为10.30.0.0/24后，10.20.0.0/24的路由被删除，访问数据包没有路由指向 wg0，无法进入隧道封装转发，最终业务访问失败。综上二者依托两套独立路径，握手状态和业务连通性互不绑定。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：1.流量全隧道路由风险：0.0.0.0/0会让本机所有IPv4流量全部生成指向 wg0 的路由，本机上网、访问任意外网/内网全走WireGuard隧道，带宽与延迟被VPN 链路占用。2.权限溢出访问风险：对端 VPN 网关侧，AllowedIPs=0.0.0.0/0代表信任客户端所有源IP，客户端可借助隧道遍历网关全内网网段，横向渗透内网全部设备，扩大攻击面。3.路由环路隐患：全量流量进隧道容易和网关默认路由形成环路，造成网络瘫痪。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能冒充客户端建立隧道。WireGuard是双向非对称密钥校验：客户端用自身私钥+对端公钥完成加密握手，VPN网关用客户端公钥校验客户端身份；攻击者仅有VPN私钥，缺少客户端私钥，无法生成合法握手报文签名。VPN的私钥仅用于解密客户端发来的握手包，不能反向伪造客户端身份、冒充客户端向VPN发起连接，因此无法仿冒client接入隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：底层underlay抓包外层源地址：192.0.2.2（client 的 underlay 底层 IP），目的地址：192.0.2.1:51820（vpn 底层 IP+WireGuard 端口）。原因：WireGuard将完整内层IP报文（源10.10.10.1、目的10.20.0.2的ICMP/HTTP）使用对称加密算法整体加密，再封装在UDP外层报文中传输；底层抓包只能捕获加密后的UDP密文载荷，原始业务数据被加密混淆，没有协商密钥无法解密还原，因此看不到内层真实IP与ICMP/HTTP明文。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：第一：地址隐藏（隐私防护）：外层仅暴露两端底层underlay公网IP，内网真实隧道IP、业务服务器IP被封装隐藏，中间人无法通过抓包获知内网拓扑与业务网段信息。第二：传输加密（内容保密）：内层原始报文被高强度加密后再封装，链路抓包只能获取密文，无法窃取ICMP、HTTP等原始业务明文数据，防止通信内容被窃听篡改。
