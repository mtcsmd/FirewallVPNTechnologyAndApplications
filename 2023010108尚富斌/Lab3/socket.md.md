...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTENING | 服务端创建、绑定、监听阶段|
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED | 客户端连接、数据收发阶段|
| `server.py` 执行 `close()` 之后 | TIME_WAIT|连接关闭、端口回收阶段 |
...
| :--- | :------- | :------- |
| 程序是否自动退出 | 是| 否|
| `ss` 中连接的 State |连接关闭（无 ESTABLISHED） | ESTABLISHED（长连接保持）|
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：客户端报 ConnectionRefusedError（目标计算机积极拒绝）。此时客户端套接字存在，但服务端监听套接字未启动，TCP 三次握手无法完成，管道不存在。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：原 server 套接字仅负责监听新连接，accept 返回新 conn 套接字用于实际数据收发，可同时处理多个客户端连接，避免阻塞监听。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符是进程内的文件标识，用于进程内定位套接字；端口号是网络层标识，用于网络中定位主机上的进程。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：未指定客户端端口，由操作系统自动分配临时端口，ss/netstat 输出中可看到客户端的临时端口（如 58606）。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：HTTP/1.0 默认短连接，服务器响应后主动关闭连接，recv() 读到 EOF（空数据），循环自动退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：HTTP/1.1 默认长连接（Connection: keep-alive），连接不会自动断开，需手动关闭；HTTP/1.0 默认短连接，响应后立即断开。
