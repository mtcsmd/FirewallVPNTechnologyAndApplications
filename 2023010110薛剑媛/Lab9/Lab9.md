...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功|返回 HTML 目录列表（显示了 .bashrc、Desktop/ 等文件列表）|
| `guest -> dmz:8080` |失败|curl: (7) Failed to connect to 10.0.3.2 port 8080 after 0 ms: Could not connect to server|
| `guest -> office` |失败|curl: (7) Failed to connect to 10.0.1.2 port 8080 after 0 ms: Could not connect to server|
...
> 答：不会。因为 iptables 规则是按顺序逐条匹配的，一旦包命中了 REJECT 规则，就会立即执行 REJECT 动作并终止该链的匹配，后面的 LOG 规则永远不会被执行。所以 LOG 必须放在 REJECT 前面，才能记录到被拒绝的包。
...
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |失败（拒绝）|是|GUEST-TO-DMZ:|
| `guest -> office` |失败（拒绝）|是|GUEST-TO-OFFICE:|
| `office -> dmz:8080` |成功|否|无|

**从终端 D 复制一条实际日志行：**

```text
[3384.669821] GUEST-TO-OFFICE: IN=veth-fw-guest OUT=veth-fw-office MAC=ae:0f:ae:64:64:5e:12:a7:4c:e0:df:00:08:00 SRC=10.0.2.2 DST=10.0.1.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=46298 DF PROTO=TCP SPT=47940 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0
```
...
> 答：合理，因为规则中没有对 ACCEPT 流量配置 LOG。如果想记录放行的流量，可以在 ACCEPT 规则前加一条 LOG 规则，例如：-A FORWARD -s 10.0.1.0/24 -d 10.0.3.2 -p tcp --dport 8080 -j LOG --log-prefix "OFFICE-TO-DMZ: "，然后再加 ACCEPT 规则。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：能。从日志中可以读出：SRC=10.0.2.2 是访问者（guest），DST=10.0.3.2 是被访问者（dmz），DPT=8080 是目标端口（Web 服务），SYN 标志表示这是一个新的 TCP 连接请求。
...
| 项目 | 你的填写 |
| :--- | :------- |
| `GUEST-TO-DMZ` 总日志条数 |12|
| `GUEST-TO-OFFICE` 总日志条数 |1|
| 循环 5 次后 `GUEST-TO-DMZ` 增加了多少 |10（从2条增加到12条）|
...
> 答：可以快速过滤出特定类型的安全事件，比如只查看访客区对 DMZ 的访问尝试，忽略其他日志。便于安全审计、故障排查和合规报告。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：会导致日志量爆炸，消耗大量磁盘空间和 CPU 资源，可能使系统响应变慢，甚至导致日志系统崩溃，丢失重要日志。
...
| :--- | :------- |
| 触发访问次数 | 10 |
| 实际写入日志条数 |不超过 5（--limit-burst 5 限制）|
| `--limit-burst 5` 的含义 |允许初始突发最多 5 条日志，之后降到每分钟 3 条|
...
> 答：LOG 和 REJECT 是独立的两个动作，LOG 只负责记录，不影响包的处理流程；REJECT 负责拦截。LOG 规则匹配后，包继续向下匹配 REJECT 规则，最终被拒绝。--limit 只限制 LOG 的写入频率，不影响 REJECT 的执行。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：有本质区别。"仍然拦截但不记录"表示攻击/违规行为仍然被阻止了，只是没有留下审计证据；而"没记录就当没发生"意味着误以为没有攻击。安全审计中，拦截是防护，记录是证据。没有记录就无法追踪攻击来源、分析攻击模式、满足合规要求。即使拦截成功，审计日志也是必要的。
...
| `guest` 地址 |10.0.2.2|
| `dmz` 地址 |10.0.3.2|
| `fw` 三个接口地址 |10.0.1.1（接office）、10.0.2.1（接guest）、10.0.3.1（接dmz）|
...
| 允许已建立连接返回 |-A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT|
| 允许 `office -> dmz:8080` |-A FORWARD -s 10.0.1.0/24 -d 10.0.3.2 -p tcp --dport 8080 -j ACCEPT|
| 拒绝 `guest -> dmz:8080` |-A FORWARD -s 10.0.2.0/24 -d 10.0.3.2 -p tcp --dport 8080 -j REJECT|
| 拒绝 `guest -> office` |-A FORWARD -s 10.0.2.0/24 -d 10.0.1.0/24 -j REJECT|
...
| :------- | :----- | :------- | :---------- |
| 1 |ACCEPT|-m conntrack --ctstate ESTABLISHED,RELATED|无|
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：如果混在同一网段，防火墙无法在 FORWARD 链上控制它们之间的访问。通过独立网段，可以在防火墙上精确控制访问权限（只允许 office 访问 dmz，拒绝 guest）。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：guest 网络不可信，可能携带恶意软件或扫描攻击。隔离可以防止访客访问办公网敏感资源。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：TCP 双向通信需要返回包。放行了 office → dmz 的 SYN（NEW），dmz 返回的 SYN+ACK 属于 ESTABLISHED，没有这条规则会被 DROP，连接无法建立。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：LOG 不终止规则匹配，只记录；ACCEPT/DROP/REJECT 会终止匹配。LOG 必须放在 REJECT 前面，否则不会被命中。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：合理。记录违规尝试用于安全审计，正常业务流量不记录以避免日志过大。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。攻击仍然被拦截，只是日志被限流以减少系统负担。防护功能不受影响。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：先接入 guest 或独立 VPN 区域。从分区角度，VPN 用户不可控，不应直接接入办公网；从审计角度，独立记录有助于追踪异常行为。
