...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功 |终端返回 Python HTTP 服务的目录列表，连接正常建立 |
| `guest -> dmz:8080` |失败 |提示 curl: (7) Failed to connect，连接被防火墙拒绝 |
| `guest -> office` |失败 |提示 curl: (7) Failed to connect，无法建立连接 |
...
| LOG 与 REJECT 的匹配条件是否完全一致 |是，源网段、目的地址、协议、端口完全相同 |

**简答：**

1. 如果把 LOG 放在 REJECT 之后，LOG 规则还会被命中吗？为什么？

> 答：不会被命中。iptables 规则是从上到下依次匹配，REJECT 属于终止类动作，数据包命中 REJECT 规则后会直接被拦截，规则匹配流程就此终止，后续的 LOG 规则永远不会被遍历和执行。因此日志无法正常记录拦截行为。
...
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |连接失败	 |是 |GUEST-TO-DMZ: |
| `guest -> office` |连接失败	 |是 |GUEST-TO-OFFICE: |
| `office -> dmz:8080` |访问成功	 |否	 |无 |
...
> 答：合理。本实验仅对违规拦截流量配置 LOG 规则，合法放行流量未添加日志规则，因此不会产生日志。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可以完整还原访问行为。日志中可读取：自定义事件标签、流量进出网卡、源 IP 地址（访问方）、目的 IP 地址（被访问方）、通信协议、源端口、目标服务端口、TCP 标志位等信息。结合以上字段，能明确判断出源主机、目标主机、使用协议、访问的服务端口，完整还原整个访问行为。
...
| 项目 | 你的填写 |
| :--- | :------- |
| `GUEST-TO-DMZ` 总日志条数 |9 |
| `GUEST-TO-OFFICE` 总日志条数 |4 |
| 循环 5 次后 `GUEST-TO-DMZ` 增加了多少 |5 |
| 你使用的过滤命令 |`sudo dmesg	、grep "GUEST-TO-DMZ"、sudo dmesg、grep "GUEST-TO-OFFICE"、sudo dmesg	、grep "GUEST-TO"	、wc -l`	 |

**简答：**

1. `--log-prefix` + `grep` 组合，相当于给不同事件打了不同"标签"。在真实场景中这有什么实际价值？

> 答：1. 分类区分事件：不同前缀对应不同违规行为，快速区分访客访问 DMZ、访客访问办公区等不同安全事件；2. 高效检索排查：生产环境内核日志体量巨大，通过前缀 + grep 可精准过滤目标日志，无需逐条翻阅；3. 运维审计统计：可按标签统计各类攻击、违规访问的频次，辅助安全分析、风险研判与故障排查；4. 日志联动告警：可基于自定义前缀对接监控平台，实现不同安全事件的差异化告警。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：1. 日志风暴：短时间产生海量日志，快速占满服务器磁盘空间，导致磁盘爆满、系统异常；2. 性能下降：频繁写入内核日志会消耗大量 CPU、IO 资源，拖累防火墙转发性能，造成网络卡顿、丢包；3. 日志失效：海量日志会淹没关键安全事件，人工或工具均难以筛选有效信息，失去审计意义；4. 服务中断：极端情况下会引发系统负载过高，导致防火墙、业务服务宕机。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5 |
| `--limit-burst 5` 的含义 |允许突发 5 条日志不做限速，应对短时间集中访问 |
| 未记录的访问是否仍然被 REJECT |是，所有 10 次访问均被拦截 |

**简答：**

1. `--limit` 只控制了 LOG 的写入频率，但后面的 REJECT 仍对每个包生效。这说明 LOG 和 REJECT 在逻辑上是什么关系？

> 答：二者是相互独立、先后执行的逻辑关系。-m limit 仅作用于 LOG 规则，只限制日志记录的频率，不会改变数据包的转发 / 拦截结果；REJECT 规则不受 limit 模块影响，每一个匹配的数据包都会被正常拦截。简单来说：LOG 负责 “选择性记录”，REJECT 负责 “无条件拦截”，两者功能解耦。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：
> 1、安全结果不同：前者不拦截、不记录，违规流量直接进入内网，产生安全风险；后者严格拦截流量，仅限制日志数量，网络安全边界不受破坏。
2、审计价值不同：后者只是精简日志，保留部分样本用于风险研判，管理员可通过少量日志判断攻击行为；前者完全丢失证据，无法追溯安全事件。
3、合规要求不同：企业安全审计只要求留存有效日志样本、阻断攻击，不要求完整记录每一条攻击包；而放任流量通行属于严重安全违规。因此在安全审计中，该区别直接关系网络安全与合规性。
...
| 允许已建立连接返回 |sudo ip netns exec fw iptables -A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT |
| 允许 `office -> dmz:8080` |sudo ip netns exec fw iptables -A FORWARD -s 10.0.1.0/24 -d 10.0.3.2 -p tcp --dport 8080 -j ACCEPT |
| 拒绝 `guest -> dmz:8080` |sudo ip netns exec fw iptables -A FORWARD -s 10.0.2.0/24 -d 10.0.3.2 -p tcp --dport 8080 -j REJECT |
| 拒绝 `guest -> office` |sudo ip netns exec fw iptables -A FORWARD -s 10.0.2.0/24 -d 10.0.1.0/24 -j REJECT |
...
| :------- | :----- | :------- | :---------- |
| 1 |ACCEPT	 |所有已建立 / 关联连接	 | 无|
| 2 |ACCEPT	 |10.0.1.0/24 → 10.0.3.2 TCP 8080	 |无 |
| 3 |LOG |10.0.2.0/24 → 10.0.3.2 TCP 8080	 |GUEST-TO-DMZ: |
| 4 |REJECT	 |10.0.2.0/24 → 10.0.3.2 TCP 8080	 |无 |
| 5 |LOG |10.0.2.0/24 → 10.0.1.0/24 所有协议	 |GUEST-TO-OFFICE: |
| 6 |REJECT |10.0.2.0/24 → 10.0.1.0/24 所有协议	 |无 |
...
| :--- | :------- |
| `GUEST-TO-DMZ` 总条数 |4 |
| `GUEST-TO-OFFICE` 总条数 | 9|
| 过滤命令 |`sudo dmesg	、grep "GUEST-TO"` |
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：DMZ 是对外提供服务的服务器区域，办公区是内部终端区域，二者网段隔离是网络安全的基础设计。
如果混为同一网段，防火墙无法通过网段区分访问主体，无法配置精细化访问策略。本实验中，依靠不同网段，我们实现了 “办公区可访问 DMZ、访客禁止访问 DMZ” 的管控；若网段合并，攻击者一旦攻陷 DMZ 服务器，可直接横向渗透到办公主机，内网数据、业务系统将全部暴露。分区隔离可以缩小安全攻击面，即使 DMZ 服务器被入侵，也能通过防火墙规则阻断其访问办公内网，保护核心数据。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：Guest 属于访客、外来人员终端，安全可信度极低，存在恶意攻击、病毒、木马、扫描探测等风险。若不与办公网隔离，访客终端一旦中毒或被控制，会直接威胁内部办公主机、业务数据安全。同时访客无严格权限管控，隔离后可通过防火墙限制其访问内网资源，遵循最小权限原则，从网络层面阻断外来风险向内网扩散。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：TCP 是面向连接的协议，访问流程为：客户端发起请求（SYN）→ 服务端响应 → 客户端接收回包。
ESTABLISHED,RELATED 用于放行已建立连接、关联连接的回包。如果删除该规则，防火墙默认丢弃所有转发流量，服务端的响应报文无法回传给客户端，TCP 三次握手无法完成，所有正常访问都会超时失败，合法业务也无法使用。该规则是保证正常网络通信的必要条件。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：
   本质区别：ACCEPT/DROP/REJECT 是终结类动作，数据包命中后规则匹配立即终止，直接决定数据包 “放行 / 丢弃 / 拒绝”；LOG 是附属类动作，仅记录日志，不会终止规则匹配，数据包会继续向下遍历其他规则。
放置原因：REJECT 是终结动作，若 LOG 放在其后，数据包被 REJECT 拦截后就不再匹配后续规则，LOG 永远无法执行，日志记录失效。因此必须将 LOG 写在 REJECT 之前，先记录行为，再拦截流量。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：属于合理设计，并非疏漏。本实验的日志定位是安全审计、记录违规行为，目标是监控非法访问、攻击探测等风险行为。Office 访问 DMZ 是预先定义的合法业务流量，无需额外记录日志，可减少日志冗余、节约系统资源。若业务需求需要审计全部流量（含合法访问），再单独为 ACCEPT 规则添加 LOG 即可。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。“不记录就当没发生” 代表流量放行、无拦截、无审计，安全边界完全失效；而 limit 限速模式下，所有违规流量依然被严格拦截，仅对日志做精简限流，只是不再重复记录高频同类攻击。网络安全防护功能正常生效，同时避免日志风暴，还保留部分日志样本用于安全分析，兼顾安全性与系统性能。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：VPN 用户应优先接入 Guest 访客区，禁止直接接入办公区。
网络分区角度：VPN 用户属于外部远程用户，安全等级等同于访客，存在未知风险。接入 Guest 区后，可沿用现有防火墙规则，强制隔离 VPN 用户与办公内网、限制其访问 DMZ 核心服务，防止外部风险入侵内网。
日志审计角度：所有 VPN 用户的访问行为都会被 Guest 区对应的 LOG 规则统一记录、分类过滤，便于集中审计、追溯远程访问行为，一旦出现异常可快速定位来源，满足安全审计要求。
