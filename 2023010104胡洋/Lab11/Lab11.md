...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |是，两端均显示27 seconds ago          |
| 是否看到 `transfer` 计数               |是，两端都有收发字节统计（client：收92B/发 360B；vpn 端：收212B/发 92B）          |
...
| 去 `10.20.0.0/24` 的路由是否消失 | 未消失，路由条目残留10.20.0.0/24 dev wg0 scope link         |
| 是否还能访问 `10.20.0.2:8080`    |不能，curl 连接超时          |
| 失败的直接原因                   | 客户端 AllowedIP 已移除10.20.0.0/24，去往该网段流量不再送入 wg 隧道，无法通过 VPN 转发         |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
wg show的握手（密钥协商）和业务转发是两套逻辑：
握手（密钥协商）：只依靠 Endpoint 底层 UDP 连通、公私钥匹配，和AllowedIP无关，所以隧道握手状态正常；
业务数据转发：由AllowedIPs控制，只有目标网段在列表内才会封装进 VPN 隧道。现在 10.20 不在AllowedIP，报文不走wg隧道，因此访问失败。
截图：
...
| 协议                     | UDP（端口 51820）                                   | RAW/IP（ICMP）         |
| 是否能看到 ICMP/HTTP 内容 | 不能，载荷是加密二进制密文                                   |可以，明文 ICMP 请求、应答报文          |
| 为什么内外两层看到的不同 |WireGuard外层UDP封装加密，底层只传输加密后的整段密文，看不到内层业务 IP 与报文                                    |wg0是隧道解密后的虚拟接口，剥离外层 UDP 包头、解密，还原原始内层明文 IP 报文          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
outer (underlay) → 信封：只记录两端底层公网 IP（信封收发地址），信封内部数据全加密；
inner (wg0) → 信纸：信纸是原始业务报文（ICMP/HTTP 明文内容、内网 IP 地址）；
安全意义：
公网链路抓包只能拿到加密密文，无法解析内网业务地址与传输数据，防止窃听、数据泄露，实现内网流量加密安全传输。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：两件事：自动生成隧道路由、校验入站源 IP；AllowedIP 优先在系统内核添加路由，决定流量是否进隧道，路由拦截在先，因此先影响路由。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手靠公私钥 + 底层 UDP 连通，与 AllowedIP 无关；业务转发由 AllowedIP 和路由管控，二者配置逻辑分离，故而独立。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0全部流量走 VPN，内网横向渗透风险陡增，VPN 节点被攻陷后可遍历全网。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。WireGuard 双向密钥校验，接入需要客户端公钥配对，仅有服务端私钥无法冒充客户端协商。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源：192.0.2.2；内层报文被 WireGuard 高强度加密，底层仅传输密文，抓包无法解密明文。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：隐藏内网真实地址、全量加密业务载荷，实现地址隐匿 + 数据防窃听。
