...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功 |连接成功，返回HTML目录 |
| `guest -> dmz:8080` |不成功 |curl 提示：Could not connect to server，连接超时被拒绝 |
| `guest -> office` |不成功 |curl 提示：Could not connect to server，连接超时被拒绝 |
...
> 答：不会，因为iptables 规则从上往下匹配，数据包命中 REJECT 后直接被拒绝，规则匹配终止，不会继续向后匹配 LOG 规则。
...
| :--- | :------- | :---------- | :------- |
| `guest -> dmz:8080` |失败 |是 |GUEST_TO_DMZ |
...
```text
5月 20 21:29:21 kali kernel: GUEST-TO-DMZ: IN=veth-fw-guest OUT=veth-fw-dmz MAC=82:54:83:7a:6f:69:de:91:5e:25:81:00:08:00 SRC=10.0.2.2 DST=10.0.3.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=17477 DF PROTO=TCP SPT=52896 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0 
...
> 答：合理,因为本实验只要求记录被拦截的违规访问。若要记录放行流量，需在对应 ACCEPT 规则前面添加 LOG 规则。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可读出源 IP、目的 IP、协议、源端口、目的端口、TCP 标志；能还原：10.0.2.2(guest) 访问 10.0.3.2(dmz) 的8080端口。
...
| 项目 | 你的填写 |
| :--- | :------- |
| `GUEST-TO-DMZ` 总日志条数 |5 |
| `GUEST-TO-OFFICE` 总日志条数 |0 |
...
> 答：可精确区分不同类型的安全事件，快速过滤、统计、告警，便于安全审计和故障排查。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：日志暴涨、磁盘占满。系统性能下降、内核压力过大，甚至导致系统卡顿或崩溃。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5 |
| `--limit-burst 5` 的含义 |允许最多突发记录5条日志 |
| 未记录的访问是否仍然被 REJECT |是，全部被拒绝 |

**简答：**

1. `--limit` 只控制了 LOG 的写入频率，但后面的 REJECT 仍对每个包生效。这说明 LOG 和 REJECT 在逻辑上是什么关系？

> 答：相互独立。LOG 仅记录日志，不改变数据包流向；REJECT 负责拦截数据包；顺序执行、互不干扰。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：有本质区别。前者放任攻击；后者仍阻断攻击，只是精简日志，兼顾安全防护与日志性能，是安全审计中常用的防护手段。
...
| :------- | :----- | :------- | :---------- |
| 1 |ACCEPT |所有，ctstate RELATED,ESTABLISHED |无 |
| 2 |ACCEPT |10.0.1.0/24→10.0.3.2 tcp:8080 |无 |
| 3 |LOG |10.0.2.0/24→10.0.3.2 tcp:8080 |GUEST-TO-DMA |
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：DMZ 是对外服务区，易被攻击；office 是内部办公网，存放敏感数据。本实验中 guest 可访问 DMZ 但不能访问 office，若混在同一网段，guest 可直接访问办公内网，造成数据泄露。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：访客网络安全不可控，易存在恶意主机、病毒、扫描行为，隔离可防止横向渗透，保护内部办公系统安全。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：TCP 连接需要回包，去掉后服务器的响应包无法返回，TCP 三次握手无法完成，所有访问都会失败。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：LOG不中断规则匹配，仅记录日志；ACCEPT/DROP/REJECT 会终止数据包流向。REJECT 会直接丢弃数据包，后面的规则无法执行，因此 LOG 必须在前。


5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：合理。实验目标是审计违规行为，正常业务流量无需记录；若需全量审计，可在 ACCEPT 前添加 LOG 规则。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有。前者防护不中断，仅精简日志；后者直接放任攻击。安全上仍有效阻断，只是优化日志性能。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：应先接入guest 区域。guest 严格隔离办公网，可通过防火墙规则控制访问权限；同时所有 VPN 访问行为可被 LOG 记录审计，安全可控。
