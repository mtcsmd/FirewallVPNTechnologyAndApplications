...
| `wan` 无法回复的原因 |wan 路由表中没有到 10.0.0.0/24 的回程路由，无法将 ICMP 回复包发回 lan 主机 |
...
conntrack 额外记录了地址映射关系：内网真实地址（10.0.0.2）与 NAT 转换后的公网地址（192.0.2.1）的对应关系。
这说明状态检测和 NAT 都是基于同一套连接跟踪（conntrack）机制实现的，它们共享连接状态信息，因此可以无缝协同工作。
2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
NAT 会为不同的内网连接分配不同的源端口（PAT，端口地址转换）。
回复包的目标地址是 NAT 公网 IP + 分配的端口号，NAT 设备根据端口号就能还原到对应的内网主机和端口。
...
| TCP 38070 访问是否成功 | 成功|
| ping 是否成功 |失败 |
| ping 失败的原因 |fw 的 FORWARD 链只允许 TCP 38070 的新连接，ICMP 包被默认 DROP 规则拦截 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址（如 10.0.0.0/8）是保留地址，不在互联网上路由，路由器不会转发目的地址为私有网段的包，因此外网无法直接访问内网主机。
NAT 让内网主机的包在出网时被转换为 NAT 设备的公网 IP，外网回复包会先到达 NAT 设备，再根据映射关系转发回内网主机，从而实现内网主机访问外网。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source <固定IP>：适合出口 IP 固定的场景（如静态公网 IP），性能略高，规则简单。
MASQUERADE：适合出口 IP 动态变化的场景（如拨号上网、DHCP 获取公网 IP），会自动跟随接口 IP 变化更新地址，无需手动修改规则。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：会给在载荷中包含 IP 地址 / 端口信息的协议带来麻烦，比如 FTP、SIP、IPsec 等。
因为 NAT 只修改 IP 头的地址，不会修改应用层载荷中的地址信息，导致载荷中的地址与实际地址不匹配，通信失败。这类协议需要 ALG（应用层网关）配合 NAT 才能正常工作。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：两者都依赖 conntrack 连接跟踪机制，共用一套状态表，集成实现可以减少资源占用、提高效率。
家用路由器通常只需要一条公网 IP，NAT 是必需功能；状态检测防火墙可以基于 conntrack 实现简单高效的访问控制，两者集成是最优选择。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：可以添加一条允许 ICMP 的新连接规则：
   sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -m conntrack --ctstate NEW -j ACCEPT
     或者更通用地允许所有 ICMP 包（更宽松）：
