...
| 测试项 | 结果 | 现象描述 |
| :----- | :--- | :------- |
| `wan → 10.0.3.2:8080` |失败 |提示网络不可达，连接超时 |
| `office → 10.0.3.2:8080` |成功 | 正常返回 HTTP 目录列表|
...
| `filter` | `FORWARD` | 1 | ACCEPT |所有已建立 / 关联连接 |允许转发 |
| `filter` | `FORWARD` | 2 | ACCEPT |dst=10.0.3.2 tcp dpt:8080 新连接 |允许转发 |
...
| 请求包的目的地址 |203.0.113.1 | 10.0.3.2|
| 回复包的源地址 |203.0.113.1 | 10.0.3.2|
...
> 答：响应包源 IP 为 203.0.113.1（防火墙公网 IP），conntrack 自动完成源地址还原，外网主机无法感知内网真实地址。

2. 如果把 DNAT 规则删掉但保留 conntrack 条目，同一条连接的后续包会怎样？

> 答：现有连接仍可正常通信。conntrack 已记录映射关系，后续数据包依靠连接跟踪自动做地址转换，仅新连接会因缺少 DNAT 规则失败。
...
> 答：LOG 规则配置在 filter 表 FORWARD 链，此时 DNAT 已完成地址改写。若要记录改写前的原始地址，需将 LOG 规则配置在 nat 表 PREROUTING 链。

2. 如果有人在扫描 `fw` 的所有端口（端口扫描），LOG 日志能帮助安全人员发现这种攻击吗？怎样从日志中识别出端口扫描行为？

> 答：可以发现。特征：同一个源 IP 在短时间内连续访问大量不同目标端口，日志中会出现固定 SRC、不断变化 DPT 的大量新连接日志，以此判定端口扫描。
...
| 项目 | 你的填写 |
| :--- | :------- |
| `wan` 地址 |203.0.113.2/24 |
| `fw` 四个接口地址 | veth-fw-office：10.0.1.1/24；veth-fw-guest：10.0.2.1/24；veth-fw-dmz：10.0.3.1/24；veth-fw-wan：203.0.113.1/24|
...
| 规则 | 表 | 链 | 匹配条件 | target |
| :--- | :--- | :--- | :--- | :--- |
| DNAT 8080 |nat |PREROUTING | -d 203.0.113.1 -p tcp --dport 8080|DNAT --to-destination 10.0.3.2:8080 |
| DNAT 2222 |nat |PREROUTING |-d 203.0.113.1 -p tcp --dport 2222 |DNAT --to-destination 10.0.3.2:2222 |
| FORWARD 8080 |filter |FORWARD |-d 10.0.3.2 -p tcp --dport 8080 --ctstate NEW |ACCEPT |
| FORWARD 2222 |filter |FORWARD |-d 10.0.3.2 -p tcp --dport 2222 --ctstate NEW |ACCEPT |

### C. 端口映射测试

| 测试项 | 结果 |
| :----- | :--- |
| `wan → 203.0.113.1:8080` |成功 |
| `wan → 203.0.113.1:2222` |成功 |
| `wan → 203.0.113.1:3306` |失败 |
| `wan → 10.0.3.2:8080` |失败 |
...
1. DNAT 改写目标地址和 SNAT 改写源地址分别发生在数据包的哪个处理阶段？如果入站和出站方向的包都需要做 NAT（例如 DMZ 既对外提供服务，又需要访问外网），PREROUTING 和 POSTROUTING 各自扮演什么角色？
   ...
   > 答：DNAT 在 nat 表 PREROUTING 链（路由决策前）改写目标地址；SNAT 在 nat 表 POSTROUTING 链（路由决策后）改写源地址。
PREROUTING：处理外网入站流量，通过 DNAT 将公网端口映射到内网 DMZ 服务；
POSTROUTING：处理内网出站流量，通过 SNAT 将 DMZ 主机地址转换为防火墙公网 IP，实现内网访问外网。

2. 如果 `fw` 本身也运行着 Web 服务（监听 `203.0.113.1:8080`），又来了一条发往 `203.0.113.1:8080` 的请求，DNAT 规则和本地服务谁会先"抢到"这个包？需要改什么配置才能让两者共存？

   > 答：DNAT 优先。PREROUTING 执行早于路由判断，地址改写后数据包会被转发到 DMZ，不会交给本机服务。
共存方案：① 使用不同公网端口区分服务；② 基于入接口做规则匹配，仅外网接口流量做 DNAT，本机访问不转换；③ 调整规则优先级。
...
3. 如果内网中有两个 Web 服务器（`10.0.3.2:8080` 和 `10.0.3.3:8080`），你想把外部的 `8080` 和 `8081` 分别映射到这两台服务器上。DNAT 的回复包经过 conntrack 逆向还原时，`fw` 是怎么知道把回复包还原到正确的"公网端口"的？
   ...
   > 答：连接跟踪基于五元组（源 IP、源端口、目的 IP、目的端口、协议）唯一区分连接。不同公网端口对应不同五元组，conntrack 会独立记录每一条映射关系，回复包根据对应条目逆向还原端口与地址，不会混淆。

4. 本实验中的端口映射没有包含 SNAT 规则（不像家用路由器那样默认做 MASQUERADE）。在这种情况下，`dmz` 服务器回复包时，源地址是 `10.0.3.2`。这个包能回到 `wan` 吗？如果能，是通过什么机制？如果不能，还需要什么配置？

   > 答：可以正常返回。DMZ 网关为防火墙内网接口，回复包先送达防火墙；依靠 conntrack 记录的 DNAT 会话，自动将回复包源地址还原为防火墙公网 IP，再转发至 wan，无需额外 SNAT 规则。

5. Lab8 的 SNAT 和 Lab10 的 DNAT 都依赖 conntrack 来追踪映射关系。如果 conntrack 表满了（比如遭受大量并发连接攻击），防火墙还能正常工作吗？会出现什么后果？有哪些防护措施？

   > 答：无法正常工作。conntrack 表满后无法新建连接条目，所有新 TCP/UDP 连接全部失败，业务中断。
防护措施：① 调大 nf_conntrack_max 连接跟踪表上限；② 缩短空闲连接超时时间，加快条目回收；③ 限制单 IP 最大并发连接数；④ 开启防攻击规则，拦截异常高频连接。

6. 从 Lab7 的有状态防火墙到 Lab8 的 NAT，再到 Lab9 的分区审计和 Lab10 的端口映射，这四次实验的共同基础是什么？如果把防火墙比作大楼的安保系统，这些功能分别对应什么？

   > 答：共同基础：Linux netfilter/iptables 框架 + conntrack 连接跟踪机制。
类比：
有状态防火墙：大楼门禁，管控进出人流、校验连接状态；
SNAT：内部人员外出统一使用大楼对外身份；
分区审计：大楼分区监控，记录各区域人员进出日志；
端口映射：指定对外接待窗口，外部访客仅能从指定入口访问指定内部区域。
